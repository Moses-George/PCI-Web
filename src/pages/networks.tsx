...
import { Plus, MapPin, Calendar, Network as NetworkIcon } from 'lucide-react';
...
const Networks = () => {
  const { data: networks, isLoading, refetch } = useGetNetworksQuery();
  const [createNetwork, { isLoading: isCreating }] = useCreateNetworkMutation();
  const [open, setOpen] = useState(false);
  const { register, handleSubmit, reset, formState: { errors } } = useForm<NetworkForm>({
    ...
    <div className="space-y-6">
        ...
        <Sheet open={open} onOpenChange={setOpen}>
          ...
          <SheetContent side="right">
            <SheetHeader>
              ...
              <SheetDescription>Fill in the details to create a new network.</SheetDescription>
            </SheetHeader>
      ...
      <div className="grid grid-cols-3 gap-4">
        ...
        <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-100">
          ...
          <p className="text-2xl font-bold">{networks?.reduce((acc, n) => acc + n.totalSections, 0) || 0}</p>
        </div>
        <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-100">
          <p className="text-sm text-gray-500">Networks Analyzed</p>
          <p className="text-2xl font-bold">{networks?.filter(n => n.totalSections > 0).length || 0}</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {networks?.map((net) => (
          <div key={net.id} className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden hover:shadow-md transition-shadow">
            <div className="h-40">
              <MapPreview center={net.coordinates} zoom={12} markers={[{ lat: net.coordinates[0], lng: net.coordinates[1] }]} height="100%" />
            </div>
            <div className="p-4">
              <h3 className="font-semibold text-lg">{net.name}</h3>
              <p className="text-sm text-gray-500 line-clamp-2">{net.description}</p>
              <div className="mt-2 flex items-center gap-3 text-xs text-gray-400">
                <span className="flex items-center gap-1"><MapPin size={14} /> {net.coordinates[0].toFixed(4)}, {net.coordinates[1].toFixed(4)}</span>
                <span className="flex items-center gap-1"><NetworkIcon size={14} /> {net.totalSections} sections</span>
              </div>
              <div className="mt-3 flex justify-between items-center">
                <span className="text-xs text-gray-400 flex items-center gap-1"><Calendar size={12} /> {new Date(net.createdAt).toLocaleDateString()}</span>
                <a
